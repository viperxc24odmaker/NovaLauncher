...
export function setupIpcHandlers(ipcMain: IpcMain): void {
  ...
  const cosmeticsService = new CosmeticsService(accountService)
  ...
  ipcMain.handle('settings:set', (_event, key: string, value: unknown) => {
    if (!['theme', 'accentColor', 'launcherVisibility', 'javaPath', 'downloadDir'].includes(key)) throw new Error(`Unknown setting: ${key}`)
  ...
  ipcMain.handle('minecraft:importMod', async (_event, instanceId: string) => {
    const result = await dialog.showOpenDialog({ properties: ['openFile'], filters: [{ name: 'Minecraft Mod', extensions: ['jar'] }] })
