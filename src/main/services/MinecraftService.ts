...
export interface PublicAccount { id: string; type: 'microsoft' | 'elyby' | 'offline'; username: string; uuid: string; avatarUrl: string | null; isActive: boolean }
...
function loadEML(): Promise<EMLLib> {
  if (!emlPromise) emlPromise = new Function('return import("eml-lib").then(module => module.default)')() as Promise<EMLLib>
  return emlPromise
}

export class MinecraftService {
  private readonly accounts: AccountService
  private readonly root = join(app.getPath('appData'), '.novalauncher')
  ...
  async loginElyBy(username: string, password: string): Promise<PublicAccount> {
    const clean = username.trim()
    if (!clean || !password) throw new Error('Enter your Ely.by username and password.')
    const EML = await loadEML()
    const result = await new EML.YggdrasilAuth('https://authserver.ely.by/auth').auth(clean, password)
    if ('needsProfileSelection' in result) throw new Error('Ely.by returned multiple profiles; profile selection is not supported yet.')
    return this.publicAccount(this.accounts.add('elyby', result))
  }

  async addOffline(username: string): Promise<PublicAccount> {
    const clean = username.trim()
    ...
    const EML = await loadEML()
    return this.publicAccount(this.accounts.add('offline', new EML.CrackAuth().auth(clean)))
  }
  ...
  async installJava(minecraftVersion: string): Promise<void> {
    if (!minecraftVersion.trim()) throw new Error('Minecraft version is required.')
    const EML = await loadEML()
    await mkdir(this.root, { recursive: true })
    await new EML.Java({ minecraft: { version: minecraftVersion }, root: this.root }).download()
  }

  async launch(input: LaunchInstanceInput): Promise<void> {
    if (!input || !input.id || !input.version) throw new Error('A valid Minecraft instance and version are required.')
    if (!['vanilla', 'fabric', 'forge', 'neoforge'].includes(input.loader)) throw new Error('Unsupported Minecraft loader.')
    ...
    const EML = await loadEML()
    const minimum = Math.max(512, Math.floor(input.memoryMin ?? 1024))
    const maximum = Math.max(minimum, Math.floor(input.memoryMax ?? 4096))
    const launcher = new EML.Launcher({
      ...
      java: { install: 'auto' },
      memory: { min: minimum, max: maximum },
  ...
  private instanceDirectory(id: string): string {
    ...
    if (!safeId) throw new Error('Invalid instance id.')
    return join(this.root, 'instances', safeId)
  }
