...
function loadEML(): Promise<EMLLib> {
  if (!emlPromise) emlPromise = new Function('return import("eml-lib")')() as Promise<EMLLib>
  return emlPromise
}
...
export class MinecraftService {
  ...
  async installJava(minecraftVersion:string):Promise<void>{
    const EML=await loadEML()
    await new EML.Java({minecraft:{version:minecraftVersion},root:ROOT_NAME}).download()
  }
  ...
  async launch(input:LaunchInstanceInput):Promise<void>{
    ...
    const launcher=new EML.Launcher({
      root:ROOT_NAME,storage:'isolated',account:stored.account,
      profile:{slug:input.id,minecraft:{version:input.version,loader:input.loader==='vanilla'?{loader:'vanilla',version:input.version}:{loader:input.loader,version:loaderVersion},args:[]}},
      cleaning:{enabled:true,ignored:['mods/','config/','saves/','resourcepacks/','shaderpacks/','logs/','crash-reports/','options.txt']},
      java:{install:'auto'},memory:{min:Math.max(512,input.memoryMin??1024),max:Math.max(1024,input.memoryMax??4096)},window:{width:1280,height:720,fullscreen:false}
    })
